...
const portfolioVideos = [
  ...
  { type:'mp4', src:'videos/sample-08.mp4', poster:'thumbnails/gallery-8.jpg', title:'UGC IA 08', subtitle:'Oferta direta' },
  { type:'mp4', src:'videos/sample-09.mp4', poster:'thumbnails/gallery-9.jpg', title:'UGC IA 09', subtitle:'Social proof' },
...
const MOVEMENT_PRESETS = {
  continuousRight: { type: 'continuous-right', speed: 67 },
...
const CAROUSEL_MOVEMENTS = {
  featured: MOVEMENT_PRESETS.continuousRight,
  ugc: MOVEMENT_PRESETS.every3Cards,
  cinematic: MOVEMENT_PRESETS.pingPong,
  reel: MOVEMENT_PRESETS.pauseStep,
  hardcopy: MOVEMENT_PRESETS.fastSlow
};
...
function setupCarousel(shell, options = {}) {
  ...
  const canMove = () => !dragging && !hoverPaused && document.visibilityState === 'visible';
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (!el || isNaN(target) || el.dataset.animated) return;
