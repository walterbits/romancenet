...
function wireMp4(cardEl, video) {
  ...
  cardEl.addEventListener('click', (e) => {
    ...
    if (isTouchDevice) return; // mobile usa tap detection abaixo
    toggleAudio();
  });

  /* ===== MOBILE: TAP DETECTION (um toque liga/desliga áudio) ===== */
  if (isTouchDevice) {
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    cardEl.addEventListener('touchend', (e) => {
      ...
      // Só ativa se foi um tap (pouco movimento e rápido)
...
function setupCarousel(shell, options = {}) {
  ...
  const endDrag = () => {
    ...
    if (moved) {
      ...
      // DESATIVA ÁUDIO AO ARRASTAR O CARROSSEL
      pauseAllVideos();
    }
  };
